...
export default function AddWatermarkPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-16 font-mono text-ink">
      {/* Header */}
      <FeatureHeader title={"Add Watermark"} descripiton={"Stamp text over your PDF pages."} />
          ...
          <div className="p-6 bg-gray-50 border-4 border-gray-200 rounded-lg space-y-6">
              ...
              <div className="flex gap-2">
                {(["tiled", "center"] as const).map((p) => (
                  <button
                    ...
                    className={`flex-1 px-4 py-2 font-bold uppercase text-sm rounded-lg border-4 transition-all ${position === p ? "border-black bg-black text-white" : "border-gray-300 hover:border-gray-400"}`}
                  >
            ...
            {position === "tiled" && (
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block font-bold text-sm uppercase tracking-wide text-gray-500 mb-2">
                    X Axis Position: {tileOffsetX}px
                  </label>
                  <input
                    type="range"
                    min="-500"
                    max="500"
                    value={tileOffsetX}
                    onChange={(e) => setTileOffsetX(Number(e.target.value))}
                    className="w-full accent-black"
                  />
                </div>
                <div>
                  <label className="block font-bold text-sm uppercase tracking-wide text-gray-500 mb-2">
                    Y Axis Position: {tileOffsetY}px
                  </label>
                  <input
                    type="range"
                    min="-500"
                    max="500"
                    value={tileOffsetY}
                    onChange={(e) => setTileOffsetY(Number(e.target.value))}
                    className="w-full accent-black"
                  />
                </div>
              </div>
              ...
              >
                {position === "tiled" ? (
                  <div
                    ...
                    style={{
                      transform: `translate(${tileOffsetX * 0.25}px, ${tileOffsetY * 0.25}px) rotate(${rotation}deg)`,
                        ...
                        {Array.from({ length: 3 }, (_, col) => (
                          <span
                            ...
                            style={{
                              ...
                              color: `rgb(${COLOR_PRESETS[colorIndex].r * 255}, ${COLOR_PRESETS[colorIndex].g * 255}, ${COLOR_PRESETS[colorIndex].b * 255})`,
                            }}
                          >
                            {text || "WATERMARK"}
                          </span>
      ...
      {result && (
        <div className="bg-surface-soft border border-hairline rounded-sm p-8 text-center space-y-6">
          ...
          <p className="text-sm text-body">"{text}" watermark applied to all pages.</p>
