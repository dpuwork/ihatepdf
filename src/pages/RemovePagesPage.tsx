...
import { downloadBlob, getPdfPageCount, removePages } from "../lib/pdf-pages";
...
export default function RemovePagesPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-16 font-mono text-ink">
      {/* Header */}
      <FeatureHeader title={"Remove Pages"} descripiton={"Select and delete specific pages from your PDF file."} />
          ...
          <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-3">
            {allPages.map((pageNum) => (
              ...
              >
                ...
                <span
                  className={`absolute bottom-0 inset-x-0 text-center text-xs font-bold py-1 ${
                    selectedPages.has(pageNum) ? "bg-red-500 text-white" : "bg-black/70 text-white"
                  }`}
                >
          ...
          <div className="flex items-center justify-between mt-6 pt-6 border-t-2 border-gray-200">
            <p className="text-gray-500">
              {selectedPages.size > 0 ? (
                <>
                  <strong className="text-red-600">{selectedPages.size}</strong> page
                  {selectedPages.size !== 1 ? "s" : ""} selected for removal
                </>
              ) : (
      ...
      {result && (
        <div className="text-center p-8 bg-green-50 border-4 border-green-500 rounded-lg">
          ...
          <p className="text-gray-600 mb-6">
            Removed {result.removedCount} page{result.removedCount !== 1 ? "s" : ""}.{" "}
            {result.remainingCount} page{result.remainingCount !== 1 ? "s" : ""} remaining.
          </p>
