...
import { downloadBlob, getPdfPageCount, rotatePages } from "../lib/pdf-pages";
...
export default function RotatePdfPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-16 font-mono text-ink">
      {/* Header */}
      <FeatureHeader title={"Rotate PDF"} descripiton={"Rotate pages within your PDF document."} />
        ...
        <>
          <div className="flex items-center justify-between mb-4">
            <p className="font-bold text-gray-600">
              {pageCount} page{pageCount !== 1 ? "s" : ""} — click to rotate 90°
            </p>
          ...
          <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-3">
            {Array.from({ length: pageCount }, (_, i) => i + 1).map((pageNum) => (
              ...
              >
                <div
                  style={{
                    transform: `rotate(${rotations[pageNum] || 0}deg)`,
          ...
          <div className="flex items-center justify-between mt-6 pt-6 border-t-2 border-gray-200">
            <p className="text-gray-500">
              {rotatedCount > 0 ? (
                <>
                  <strong className="text-blue-600">{rotatedCount}</strong> page
                  {rotatedCount !== 1 ? "s" : ""} will be rotated
                </>
              ) : (
                "Click pages to rotate them"
              )}
            </p>
