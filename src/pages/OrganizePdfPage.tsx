...
import { downloadBlob, getPdfPageCount, removePages, reorderPages } from "../lib/pdf-pages";
...
export default function OrganizePdfPage() {
  ...
  const isModified = () => {
    ...
    return (
      visiblePages.some((p, i) => p !== original[i]) || visiblePages.length !== original.length
    );
  };
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-16 font-mono text-ink">
      {/* Header */}
      <FeatureHeader title={"Organize PDF"} descripiton={"Drag and drop to reorder pages, or delete pages you don't need."} />
      ...
      {file && pageCount && !result && !isProcessing && (
        <>
          <p className="font-bold text-gray-600 mb-4">
            {pageCount} page{pageCount !== 1 ? "s" : ""} — drag to reorder, click ✕ to remove
          </p>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3">
            {visiblePages.map((pageNum, index) => (
              <div
                ...
                className={`relative group rounded-lg overflow-hidden border-4 cursor-grab active:cursor-grabbing transition-all ${
                  dragOverIndex === index
          ...
          {deletedPages.size > 0 && (
            <div className="mt-6 p-4 bg-red-50 border-2 border-red-200 rounded-lg">
              <p className="font-bold text-red-600 text-sm mb-2">
                Deleted pages (click to restore):
              </p>
            ...
            <p className="text-gray-500">
              {isModified() ? (
                <>
                  {deletedPages.size > 0 && (
                    <>
                      <strong className="text-red-600">{deletedPages.size}</strong> deleted ·{" "}
                    </>
                  )}
