...
export default function PdfToJpgPage() {
    ...
    try {
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        const blob = await new Promise<Blob>((resolve) => {
          canvas.toBlob((b) => resolve(b!), mimeType, format === "jpeg" ? quality : undefined);
        });
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-16 font-mono text-ink">
      {/* Header */}
      <FeatureHeader title={"PDF to JPG"} descripiton={"Extract pages from your PDF as high-quality images."} />
                ...
                <div className="flex gap-2">
                  {(["jpeg", "png"] as const).map((f) => (
                    <button
                      key={f}
                      onClick={() => setFormat(f)}
                      className={`flex-1 px-4 py-2 font-bold uppercase text-sm rounded-lg border-4 transition-all ${format === f ? "border-black bg-black text-white" : "border-gray-300 hover:border-gray-400"}`}
                    >
                      {f === "jpeg" ? "JPG" : "PNG"}
                    </button>
                  ))}
                ...
                <div className="flex gap-2">
                  {DPI_OPTIONS.map((d) => (
                    <button
                      key={d}
                      onClick={() => setDpi(d)}
                      className={`flex-1 px-3 py-2 font-bold text-sm rounded-lg border-4 transition-all ${dpi === d ? "border-black bg-black text-white" : "border-gray-300 hover:border-gray-400"}`}
                    >
                      {d}
                    </button>
                  ))}
                  ...
                  <div className="flex gap-2">
                    {QUALITY_OPTIONS.map((q) => (
                      <button
                        ...
                        className={`flex-1 px-3 py-2 font-bold text-sm rounded-lg border-4 transition-all ${quality === q.value ? "border-black bg-black text-white" : "border-gray-300 hover:border-gray-400"}`}
                      >
                        {q.label}
                      </button>
                    ))}
      ...
      {isProcessing && (
        <ProgressIndicator
          progress={progress}
          status={`Converting page ${Math.ceil((progress / 100) * (pageCount || 1))} of ${pageCount}...`}
        />
      )}
