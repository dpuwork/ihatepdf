...
export default function AddPageNumbersPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-16 font-mono text-ink">
      {/* Header */}
      <FeatureHeader title={"Add Page Numbers"} descripiton={"Add numbering to your PDF pages."} />
              ...
              <div className="grid grid-cols-3 gap-2 max-w-xs">
                {POSITIONS.map((pos) => (
                  <button
                    ...
                    className={`px-3 py-2 text-xs font-bold uppercase rounded-lg border-4 transition-all ${
                      position === pos.value
              ...
              <div className="flex flex-wrap gap-2">
                {FORMATS.map((f) => (
                  <button
                    ...
                    className={`px-4 py-2 text-sm font-bold rounded-lg border-4 transition-all ${
                      format === f.value
                        ? "border-black bg-black text-white"
                        : "border-gray-300 hover:border-gray-400"
                    }`}
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              <div>
                ...
                <input
                  ...
                  value={fontSize}
                  onChange={(e) =>
                    setFontSize(Math.max(6, Math.min(48, parseInt(e.target.value) || 12)))
                  }
              ...
              <div>
                ...
                <input
                  ...
                  value={margin}
                  onChange={(e) =>
                    setMargin(Math.max(5, Math.min(100, parseInt(e.target.value) || 30)))
                  }
                  className="w-full px-3 py-2 font-bold border-4 border-black rounded-lg"
                  min="5"
                  max="100"
      ...
      {result && (
        <div className="text-center p-8 bg-green-50 border-4 border-green-500 rounded-lg">
          ...
          <p className="text-gray-600 mb-6">
            Numbers added at {position.replace("-", " ")} position.
          </p>
